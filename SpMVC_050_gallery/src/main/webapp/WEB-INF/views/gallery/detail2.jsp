...
div.gallery_file:after {
	...
	transition: 1s;
	/*
	box내에 있는 text의 그려지는 높이를 box의 높이와 같게 만들면
...
div.gallery_file:hover:after {
	content: '\f2ed';
	font-size: 30px;
	font-family: "Font Awesome 5 Free"; background-color : rgba( 0, 0, 0,
	0.3);
	color: white;
...
delete_button.addEventListener("click",()=>{
	if(confirm("일련번호 ${GALLERY.g_seq} 인 게시물 삭제 ??")) {
		...
		동시성처리(멀티 환경에서 서로 변수가 간섭하는 현상을 핸들링)을 쉽게 하기위한 방안으로 사용한다.
		상수를 선언하는 이유
...
if(gallery_files) {
	gallery_files.addEventListetener("click", (e)=> {
		const tag = e.taget
		...
		//alert(tag.tagname) // 이벤트가 어디에 걸리는지 확인
		...
		if(tag.tagName === "DIV" && tag.className.includes("gallery_file")) {
			...
			if(confirm(seq + "이미지 삭제")) {
				...
				//location을 쓰지않고 fetch를 쓰는 이유는 삭제하고 원래 화면으로 보여야 하기 때문
				fetch("${rootPath}/gallery/file/delete/" + 'A001')
				.then(response =>response.text()) // 서버가 보내면 나는 문자열로 받겠다.
				.then(result => {
					if(result === "OK") {
						alert("삭제성공")
						/*현재 클릭된 DIV tag 요소를 화면에서 제거하라 데이터베이스는 남아있음 */
						tag.remove()
