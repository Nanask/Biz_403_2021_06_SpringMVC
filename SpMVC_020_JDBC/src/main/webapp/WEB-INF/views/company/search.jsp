			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach items="${COMPS}" var="COMP" varStatus="seq">
						<tr data-cp_ccode="${COMP.cp_code}" class="search_comp">
							<td>${COMP.cp_code}</td>
							<td>${COMP.cp_title}</td>
							<td>${COMP.cp_ceo}</td>
							<td>${COMP.cp_tel}</td>
...
<script>
	document.querySelector("section#search_sec table").addEventListener("click",()=> {
		...
		if(tagName === "TD") {
			...
			TD가 click되면 어떤 데이터를 가져오고 싶다.
			TD는 여러개가 있으므로 어떤 TD가 click될지 알 수 없다.
			한개의 TD에만 데이터를 담아두고 그 TD만 클릭했을 때 반응하도록 하면 사용자가 상당히 불편할 것이다.
			그래서 TD가 선택되면(click 되면) TD를 감싸고 있는 TR이 무엇인지 알아보고 selector하라 = e.target.closest("TR")(나를 감싸고 있는 TR)
			*/
