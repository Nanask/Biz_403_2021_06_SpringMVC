			...
			<fieldset>
				...
				<div>
					<label>출판사</label> <input name="bk_ccode" id="bk_ccode"
						placeholder=""> <span id="cp_title" class="name">출판사명</span>
				</div>
				<div>
					<label>저자</label> <input name="bk_acode" id="bk_acode"
						placeholder=""> <span id="au_name" class="name">저자명</span>
				</div>
				<div>
	...
	document.addEventListener("click",(e)=> {
		...
		if(tagName === "TD") {
			...
			if(parentClassName === "search_comp") {
				...
				// 현재 선택된 tr의 child를 가져와서 각각 TD칼럼에 담겨있는 값을 추출
				...
				documemt.querySeletor("input#bk_ccode").value = ccode
				document.querySeletor("span#cp_title").innerText = ctitle
				...
				documemt.querySeletor("input#bk_acode").value = acode
				document.querySeletor("span#au_name").innerText = msg
			}
				//alert("저자코드 : " + acode)		
